...
def render_modifier_settings(settings_containers):
    ...
    with settings_containers["modifier_settings"]:
        ...
        if not WidgetRegistry.is_registered(widget_name="enable_db", widget_category="checkbox"):
...
def update_active_problem_types():
    ...
    selections = sorted(WidgetRegistry.get_widget_value("problem_types", "segmented_control"))
...
def make_number_boxes(type_, position_):
    get_val = lambda key_: WidgetRegistry.get_widget_value(widget_name=key_, widget_category="number_input_box")
...
def is_start_button_disabled():
    ...
    current_duration_value = WidgetRegistry.get_widget_value("duration", "number_input_box")
    current_duration_option = WidgetRegistry.get_widget_value("duration", "segmented_control")
